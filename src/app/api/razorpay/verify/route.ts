  ...
  try {
    ...
    if (!billingCheck.isValid) {
      return NextResponse.json(
        {
          error: 'Valid billing details are required to complete your order.',
          fieldErrors: billingCheck.errors
        },
        { status: 400 }
      )
    }
    ...
    const vaultEntries = items.map((item: any, index: number) => {
      ...
      return {
        ...
        amount: finalPrice,
        currency: 'INR',
        payment_gateway: isFree ? 'free' : 'razorpay',
    ...
    after(async () => {
      try {
        ...
        if (billingDetails) {
          const cleanDetails = billingCheck.sanitized
