...
export function BackgroundMural() {
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 -z-10 overflow-hidden pointer-events-none select-none bg-black">
      ...
      {/* LAYER 1: Comic Halftone Dots Overlay - Disabled on mobile to avoid constant SVG pattern recalculations */}
      {!isMobile && (
        <motion.div 
          style={{ 
            y: dotsY,
      ...
      {/* LAYER 2: Floating Comic Speed Lines - Disabled on mobile to avoid DOM overhead */}
      {!isMobile && (
        <motion.div 
          style={{ 
            y: speedLinesY,
      ...
      {/* LAYER 3: Floating Musical Assets (Notes, etc.) - Disabled on mobile to avoid SVG transform rendering overhead */}
      {!isMobile && (
        <motion.div 
          style={{ 
            y: assetsY,
      ...
      {/* LAYER 5: Floating Neon Graffiti Tags - Disabled on mobile to avoid extremely slow CSS drop-shadow blur calculations */}
      {!isMobile && (
        <motion.div 
          style={{ 
            y: tagsY,
      ...
      {/* LAYER 5.5: Hand-Drawn Comic Doodles (Stars, Crowns) - Disabled on mobile to avoid DOM overhead */}
      {!isMobile && isClient && (
